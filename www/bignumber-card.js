class BigNumberCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.isNoneConfig = Boolean(
      cardConfig.noneString ||
        cardConfig.noneCardClass ||
        cardConfig.noneValueClass
    );
    ...
    if (entityState !== this._entityState) {
      if (config.min !== undefined && config.max !== undefined) {
        root
          ...
          .style.setProperty(
            "--bignumber-percent",
            `${this._translatePercent(entityState, config.min, config.max)}%`
          );
      }
      root
        .querySelector("ha-card")
        .style.setProperty(
          "--bignumber-fill-color",
          `${this._getStyle(entityState, config)}`
        );
      root
        .querySelector("ha-card")
        .style.setProperty(
          "--bignumber-color",
          `${this._getColor(entityState, config)}`
        );
      if (config.hideunit == true) {
        ...
        root.getElementById(
          "value"
        ).textContent = `${entityState} ${measurement}`;
      }
      this._entityState = entityState;
      let value =
        config.round == null
          ? entityState
          : parseFloat(entityState).toFixed(config.round);
